---
title: "B5: From Spec to Working Code"
---

<div class="lesson-badges"><span class="badge group-b">Group B</span><span class="badge-time">5 min</span></div>

# B5: From Spec to Working Code

This is the payoff: your code now follows a clear requirement contract.

<div class="best-practice">
<strong>Prompting pattern</strong><br>
"Implement Feature#101. Follow the acceptance criteria and edge cases from the issue."
</div>

<div class="exercise">
<ol>
<li>Copy one Feature spec from your issues.</li>
<li>Ask Copilot to implement it in your starter repo.</li>
<li>Review output against acceptance criteria.</li>
</ol>
</div>

<div class="checkpoint">
<label><input type="checkbox" class="checkpoint-check"> I generated code from a spec and validated it against requirements.</label>
</div>

<div class="navigation-footer">
  <a class="btn btn-secondary" href="./B4-skills-specs">← Previous</a>
  <a id="next-btn" class="btn btn-primary" href="./B6-comparison">Next →</a>
</div>
